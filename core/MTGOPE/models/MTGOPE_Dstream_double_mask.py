...
class MTGOPE_Dstream_DoubleMask(nn.Module):
    def __init__(self, cfg, backbone, depth_backbone, geo_head_net, neck=None, pnp_net=None):
        super().__init__()
        assert cfg.MODEL.POSE_NET.NAME == "MTGOPE_Dstream_double_mask", cfg.MODEL.POSE_NET.NAME
    ...
    def forward(
        ...
        assert x.shape[1] > 3, x.shape  # should be rgb (3) + depth (>=1)

        rgb_feat = self.backbone(x[:, :3, :, :])  # [bs, c, 8, 8]
        if isinstance(rgb_feat, (list, tuple)):
            rgb_feat = rgb_feat[0]
        depth_feat = self.depth_backbone(x[:, 3:, :, :])
        if isinstance(depth_feat, (list, tuple)):
            depth_feat = depth_feat[0]
        conv_feat = torch.cat([rgb_feat, depth_feat], dim=1)
...
def build_model_optimizer(cfg, is_test=False):
    ...
    # depth backbone
    depth_backbone_cfg = net_cfg.DEPTH_BACKBONE
    assert depth_backbone_cfg.ENABLED
    init_depth_backbone_args = copy.deepcopy(depth_backbone_cfg.INIT_CFG)
    depth_backbone_type = init_depth_backbone_args.pop("type")
    if "timm/" in depth_backbone_type or "tv/" in depth_backbone_type:
        init_depth_backbone_args["model_name"] = depth_backbone_type.split("/")[-1]

    depth_backbone = BACKBONES[depth_backbone_type](**init_depth_backbone_args)
    if depth_backbone_cfg.FREEZE:
        for param in depth_backbone.parameters():
            with torch.no_grad():
                param.requires_grad = False
    else:
        params_lr_list.append(
            {"params": filter(lambda p: p.requires_grad, depth_backbone.parameters()), "lr": float(cfg.SOLVER.BASE_LR)}
        )
    ...
    # build model
    model = MTGOPE_Dstream_DoubleMask(
        cfg, backbone, depth_backbone=depth_backbone, neck=neck, geo_head_net=geo_head, pnp_net=pnp_net
    )
    if net_cfg.USE_MTL:
        params_lr_list.append(
            {
