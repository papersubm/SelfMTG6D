...
class MTGOPE_Evaluator(DatasetEvaluator):
...
def MTGOPE_inference_on_dataset(cfg, model, data_loader, evaluator, amp_test=False):
    ...
    with inference_context(model), torch.no_grad():
        for idx, inputs in enumerate(data_loader):
            ...
            with autocast(enabled=amp_test):  # MTGOPE amp_test seems slower
                out_dict = model(
                    batch["roi_img"],
...
def MTGOPE_save_result_of_dataset(cfg, model, data_loader, output_dir, dataset_name, train_objs=None, amp_test=False):
    """
