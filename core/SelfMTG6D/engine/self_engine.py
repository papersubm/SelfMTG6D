...
from core.SelfMTG6D.datasets.data_loader_self import build_MTGOPE_self_train_loader
from core.SelfMTG6D.datasets.data_loader import build_MTGOPE_train_loader, build_MTGOPE_test_loader
from core.SelfMTG6D.losses.ssim import SSIM, MS_SSIM
from core.SelfMTG6D.losses.perceptual_loss import PerceptualLoss

from .MTGOPE_engine_utils import batch_data, get_out_coor, get_out_mask
from .self_engine_utils import batch_data_self, compute_self_loss
from .MTGOPE_evaluator import MTGOPE_inference_on_dataset, MTGOPE_Evaluator, MTGOPE_save_result_of_dataset
from .MTGOPE_custom_evaluator import MTGOPE_EvaluatorCustom
import ref
...
def get_evaluator(cfg, dataset_name, output_folder=None):
    ...
    if evaluator_type == "bop":
        MTGOPE_eval_cls = MTGOPE_Evaluator if cfg.VAL.get("USE_BOP", False) else MTGOPE_EvaluatorCustom
        return MTGOPE_eval_cls(
...
def do_save_results(cfg, model, epoch=None, iteration=None):
    ...
    for dataset_name in cfg.DATASETS.TEST:
        ...
        data_loader = build_MTGOPE_test_loader(cfg, dataset_name, train_objs=train_obj_names)
        MTGOPE_save_result_of_dataset(
            cfg,
            model,
            data_loader,
...
def do_test(cfg, model, epoch=None, iteration=None):
    ...
    for dataset_name in cfg.DATASETS.TEST:
        ...
        data_loader = build_MTGOPE_test_loader(cfg, dataset_name, train_objs=evaluator.train_objs)
        results_i = MTGOPE_inference_on_dataset(cfg, model, data_loader, evaluator, amp_test=cfg.TEST.AMP_TEST)
...
def do_train(
    ...
    data_loader = build_MTGOPE_self_train_loader(cfg, train_dset_names, train_objs=obj_names)
    ...
    if train_2_ratio > 0.0 and len(train_2_dset_names) > 0:
        data_loader_2 = build_MTGOPE_train_loader(cfg, train_2_dset_names)
