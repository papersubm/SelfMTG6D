...
class MTGOPE(nn.Module):
    def __init__(self, cfg, backbone, geo_head_net, neck=None, pnp_net=None):
        super().__init__()
        assert cfg.MODEL.POSE_NET.NAME == "MTGOPE", cfg.MODEL.POSE_NET.NAME
    ...
    def forward(
        ...
        if do_self:
            ...
            if mask_atten is not None:
                ...
                mask_prob = get_mask_prob(mask, mask_loss_type=net_cfg.LOSS_CFG.MASK_LOSS_TYPE)
            out_dict.update(
                {
                    "mask": mask,
                    "mask_prob": mask_prob,
...
def build_model_optimizer(cfg, is_test=False):
    ...
    # build model
    model = MTGOPE(cfg, backbone, neck=neck, geo_head_net=geo_head, pnp_net=pnp_net)
    if net_cfg.USE_MTL:
        params_lr_list.append(
            {
                "params": filter(
