...
if __name__ == "__main__":
    MTGOPE_res_path = osp.join(
        PROJ_ROOT,
        "datasets/BOP_DATASETS/ycbv/test/init_poses/",
        "resnest50d_AugCosyAAEGray_BG05_visib10_mlBCE_DoubleMask_ycbvPbr100e_SO_ycbvTrainRealUw_MTGOPEPose_withYolov4PbrBbox.json",
    )
    new_res_path = osp.join(
        PROJ_ROOT,
        "datasets/BOP_DATASETS/ycbv/test/init_poses/",
        "resnest50d_AugCosyAAEGray_BG05_visib10_mlBCE_DoubleMask_ycbvPbr100e_SO_MTGOPEPose_wBboxCrop_wCosyPose_ycbvTrainRealUw.json",
    )
    if osp.exists(new_res_path):
        wprint("{} already exists! overriding!".format(new_res_path))

    bases = mmcv.load(MTGOPE_res_path)
    results = mmcv.load(res_path)["MTGOPE_init/refiner/iteration=2"]
